...
function Title() {
    ...
    const deleteRowData = (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                http.delete(`/titles/${id}`).then(res => {
    ...
    const columns = [
        ...
        {
            title: "Action", field: "patient", render: (row) => <div><Link to={`/edit-titles/${row.id}`} class="btn btn-sm action-btn"><i class="far fa-edit"></i></Link>&nbsp;<button onClick={(e) => deleteRowData(e, row.id)} className="btn btn-sm action-btn"> <i class="far fa-trash"></i> </button></div>,
            cellStyle: {
                textAlign: "center",
            },
        },
    ...
    return (
        <div className="page-content" >
            <div className="custom-card patients-head ">
                <h5 className="fw-normal custom_py-3 px-2  text-start mb-2 card-title">Title List
                <Link className="btn btn-sm btn-primary float-end"  to="/add-titles">
