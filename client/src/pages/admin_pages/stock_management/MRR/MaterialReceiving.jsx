...
function MaterialReceiving() {
    ...
    useEffect(() => {
        http.get(`/material-receiving`).then(res => {
            setSpinner(false);
    ...
    const proceedToApproval = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                http.post(`/proceed-to-mrr-copy/${id}`, formData).then(res => {
    ...
    const columns = [
        ...
        {
            title: "Material Receiving No", field: `mrr_no`
        },
        // {
            ...
            <div>
                {
                    row.mrr_status === 'New' 
                    ?
                    <span title='New MRR' className="btn-success-custom">
        ...
        },
        {
            ...
            <div>

                {
                    ...
                    ?
                    <Link title='Edit Mrr' to={`/edit-material-receiving/${row.id}`} class="btn btn-sm action-btn">
                ...
                
                {
                    row.mrr_status !== 'New' 
                    ?
                    <Link title='View Mrr' to={`/view-material-receiving/${row.id}`} class="btn btn-sm action-btn">
    ...
    return (
        <div className="page-content">
            <div className="custom-card patients-head ">
                <h5 className="fw-normal custom_py-3 px-2  text-start mb-2 card-title">Material Receiving List
                    <Link className="btn btn-sm btn-primary float-end" to="/add-material-receiving">
